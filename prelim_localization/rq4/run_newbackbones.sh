...
for M in "tencent/Hunyuan-A13B-Instruct" "inclusionAI/Ling-flash-2.0"; do
  ...
  echo "===== $(date '+%T') $M : passA_nocov (clean Static) ====="
  OPENAI_BASE_URL="$T2_BASE_URL" OPENAI_API_KEY="$T2_API_KEY" MODEL="$M" SWEBENCH_DATASET=lite \
    ...
    --instances "$D/S1_ids.json" --reuse-files "$D/oracle_files_S1.json" --dump-substrate \
    > "$D/passA_nocov.log" 2>&1 || { echo "!! passA FAIL: $M"; continue; }
  echo "===== $(date '+%T') $M : passB_rerun (cov-narrow M4) ====="
  OPENAI_BASE_URL="$T2_BASE_URL" OPENAI_API_KEY="$T2_API_KEY" MODEL="$M" SWEBENCH_DATASET=lite \
    python3 egl_e2e.py --out "$D/passB_rerun.json" --k 5 --topk 10 --workers 4 --cov-workers 2 --arise-gold \
    --cov-narrow --instances "$D/S1_ids.json" --reuse-files "$D/oracle_files_S1.json" --dump-substrate \
    > "$D/passB_rerun.log" 2>&1 || { echo "!! passB FAIL: $M"; continue; }
  echo "===== $(date '+%T') $M DONE ====="
done
echo "===== $(date '+%T') ALL NEW BACKBONES DONE ====="
