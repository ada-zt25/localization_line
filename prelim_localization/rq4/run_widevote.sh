...
source .t2_secrets
export EGL_WIDE_VOTE=1
for M in "tencent/Hunyuan-A13B-Instruct" "inclusionAI/Ling-flash-2.0"; do
  tag=$(echo "$M" | tr '/' '_'); D="rq4/results_${tag}"
  echo "===== $(date '+%T') $M passA_nocov WIDE ====="
  OPENAI_BASE_URL="$T2_BASE_URL" OPENAI_API_KEY="$T2_API_KEY" MODEL="$M" SWEBENCH_DATASET=lite EGL_WIDE_VOTE=1 \
    python3 egl_e2e.py --out "$D/passA_widevote.json" --k 5 --topk 10 --workers 4 --cov-workers 2 --arise-gold \
  ...
  echo "===== $(date '+%T') $M passB WIDE ====="
  OPENAI_BASE_URL="$T2_BASE_URL" OPENAI_API_KEY="$T2_API_KEY" MODEL="$M" SWEBENCH_DATASET=lite EGL_WIDE_VOTE=1 \
    python3 egl_e2e.py --out "$D/passB_widevote.json" --k 5 --topk 10 --workers 4 --cov-workers 2 --arise-gold \
  ...
  echo "===== $(date '+%T') $M WIDE DONE ====="
done
echo "===== $(date '+%T') ALL WIDE-VOTE DONE ====="
