...
- **6 个真实、低流行度库**(逐字 vendored,纯本地可跑),跨数据/配置/插件/提取/持久化五个领域:`simplug 0.5.7`(插件)、`diot 0.3.4`(属性字典)、`simpleconf 0.9.3`(分层配置)、`glom 25.x`(嵌套数据提取)、`bidict 0.23.x`(双向映射)、`sqlitedict 2.1.0`(持久化存储)。simplug/diot/simpleconf/glom 用于"未见→收益"主张;**bidict 作为高先验对照**(模型已熟,B4 无收益,印证"收益依赖未见程度");**sqlitedict 提供第 6 类 completion-obligation**(写入必须 `commit()` 才持久,违规运行时可观察)。**不含自编玩具库**——经科学性复核,自编库存在构造效度/循环性问题,不能作为主证据(见 §6)。
...
**现状**:**63 个任务**(simplug 10、diot 12、simpleconf 18、glom 10、bidict 10、sqlitedict 3),全部锚定自检通过(simplug 42 / diot 25 / simpleconf 36 / glom 20 / bidict 20 / sqlitedict 10 = **153 例**)。这些库覆盖 **全部 6 类 pair**(param-dependency 11 / config-return-contract 13 / return-flow 12 / shared-receiver 12 / lifecycle 12 / completion-obligation 3);completion-obligation 由 sqlitedict(写入→`commit()` 的持久化义务)补齐。**全部 6 库统一跑在通用 benchlib 引擎**(`lib_<name>.py` + `benchlib_*.py`)。
...
> 对应代码:`exec_oracle/lib_<lib>.py`(`FUNC_NAMES`/`INPUTS`/`CHECKS`)+ `fixture_<lib>.py`;判分核心 `exec_oracle/benchlib.py::evaluate_source`(全部 6 库统一)。
...
| completion-obligation | `obligation_unmet` | sqlitedict 写入(`db[k]=v`/`update`/`del`)必须配对 `commit()` 才持久(`close()`/`with` 都不兜底);漏 commit 由新连接读不到判定 |

> 说明:glom 是无状态的提取库,**天然无 lifecycle**(如实省略,见 §3.2);simplug/diot/simpleconf/bidict 覆盖含 lifecycle 在内的 5 类;sqlitedict 专供 completion-obligation。六库合并覆盖**全部 6 类**。

### 2.4 范围界定
显式排除并列 future work:n 元顺序/优先级、声明级型/协议一致(async/签名)、跨库全局配置。
> 对应代码:gold 规则文本——`exec_oracle/lib_<lib>.py::GOLD_PAIR_RULES`(每条标注真实语义/源码出处)。
...
### 3.2 六库一览(全部真实、vendored,统一 benchlib 接口)

| 库 | 版本 | 领域 | 覆盖的 pair 类型 | B0 先验 | 角色 | 对应代码 |
|---|---|---|---|---|---|---|
| **simplug** | 0.5.7 | 插件/扩展 | param/sr/rf/lifecycle/config-return | 低(~0.20) | 主(未见) | `lib_simplug.py` + `fixture_simplug.py`(复用 `hidden_tests.py`/`anchor_solutions.py`) |
...
| **sqlitedict** | 2.1.0 | 持久化存储 | completion-obligation | 低(约定特定:写入须 commit) | **第 6 类补足** | `fixture_sqlitedict.py`、`lib_sqlitedict.py` |

> 五个主库覆盖 **B0 先验强度全谱**(simpleconf 0.03 → glom 0.15 → simplug 0.20 → diot 0.65 → bidict 0.85),比"全是未见库"更能支撑"B4 收益 ∝ 未见程度"的主张;sqlitedict 专供第 6 类约束(其 commit 义务是库特定的反直觉语义,先验低)。
> **退役记录**:pyparam 0.5.4 曾在册,经实测 B1(纯 API 清单)即满分、B4−B1≈0——其 `add_param/parse/ns.attr` 语义被 API 清单完全表达,且自动类型推断使非平凡候选无干净 oracle,**于 6.15 移除**(数据归档 `result/_archive/`,`lib_pyparam.py`/`fixture_pyparam.py` 代码于 6.16 一并删除)。varname 曾作候选,因靠帧/源码魔法、仅契合 3 类 pair、exec 下需 hack 而弃用。

### 3.3 vendoring(复现)
仓库不提交 `vendor/`(48M 第三方库 + 平台专用 .so);用一条命令重建全部 6 库到 `exec_oracle/vendor/`(注意 simpleconf 的 PyPI 包名是 `python-simpleconf`,导入名才是 `simpleconf`):
```bash
cd Ollama问题存在性实验/exec_oracle
python3 -m pip install --target vendor \
  simplug==0.5.7 diot==0.3.4 python-simpleconf==0.9.3 glom==25.12.0 bidict==0.23.1 sqlitedict==2.1.0
```
...
| sqlitedict | `sqlitedict` | `sqlitedict` | 2.1.0 |
...
> 对应代码:`exec_oracle/benchlib_generate.py::method_payload`(`gold_rule_for_task` 按 pair_type 选规则 + `_split_pair_rules` 按 `Relation:` 标签切分整块 GOLD_PAIR_RULES,`API_LIST ⊕ 规则`)——全部 6 库统一。

### 4.2 难度分层
easy(单约束/无干扰)、medium(主约束+诱导项)、hard(多约束相互作用/反直觉)。产物含 `pair_type`/`difficulty`,结果可按类型/难度切片(`benchlib_eval.py::aggregate`)。

### 4.3 任务目录(63 任务,全部锚定通过)
...
**sqlitedict**(3,`lib_sqlitedict.py`):completion-obligation——SD001 durable_put(easy)/ SD002 durable_update(medium)/ SD003 durable_delete(medium):写入、更新、删除均须 `commit()` 才持久,违规由"新连接读不到"判定(`close()`/`with` 都不兜底)。

### 4.4 分布表

**pair-type × 库(63,6 类全覆盖)**:

| 类型 | simplug | diot | simpleconf | glom | bidict | sqlitedict | 合计 |
|---|---:|---:|---:|---:|---:|---:|---:|
| param-dependency | 2 | 3 | 2 | 2 | 2 | 0 | **11** |
| shared-receiver | 2 | 2 | 4 | 2 | 2 | 0 | **12** |
| return-flow | 2 | 2 | 3 | 3 | 2 | 0 | **12** |
| lifecycle | 2 | 2 | 6 | 0 | 2 | 0 | **12** |
| config-return-contract | 2 | 3 | 3 | 3 | 2 | 0 | **13** |
| completion-obligation | 0 | 0 | 0 | 0 | 0 | 3 | **3** |
| **合计** | **10** | **12** | **18** | **10** | **10** | **3** | **63** |

**难度**:easy 17 / medium 38 / hard 8。

> completion-obligation 由 **sqlitedict**(持久化存储,写入须 `commit()` 的义务)覆盖,3 个任务;可按需扩到更多资源型库(连接/会话/锁)以增样本。
...
> 判分核心:`exec_oracle/benchlib.py::evaluate_source`(全部 6 库)。
...
### 5.3 锚定套件(可信度硬门槛)
判分前过锚定:canonical 正确解 + **静态会误判的语义等价变体** 必过;每类违规模式 必挂且 reason 符合预期。**不全绿,评估拒绝运行**。当前(全过):**simplug 42 / diot 25 / simpleconf 36 / glom 20 / bidict 20 / sqlitedict 10 = 153 例**。
> 代码:`lib_<lib>.py::ANCHORS` + `benchlib.py::run_anchors`(命令 `python3 benchlib.py <lib>`;simplug 复用 `anchor_solutions.py`);评测前门控 `benchlib_eval.py`。
...
> 代码:`benchlib_runner.py`(全部 6 库的子进程隔离判分)。
...
- **诚实的覆盖边界**:数据/配置/提取库本身不含 completion-obligation,故专门引入**真实资源型库 sqlitedict**(写入→`commit()` 义务)补足第 6 类,而非在原库上硬造;glom 无 lifecycle 仍如实省略(强行制造会重蹈"自编约束"覆辙)。
...
| 组件 | 文件 | 作用 |
|---|---|---|
| simplug 模块(benchlib) | `exec_oracle/lib_simplug.py` + `fixture_simplug.py`(复用 `hidden_tests.py`/`anchor_solutions.py`) | simplug 的统一 benchlib 接口:TASKS/B0–B4/CHECKS/ANCHORS(判据与锚定沿用已验证的 hidden_tests/anchor_solutions) |
| sqlitedict 模块(benchlib) | `exec_oracle/lib_sqlitedict.py` + `fixture_sqlitedict.py` | **completion-obligation 库**(第 6 类):写入必须 `commit()` 才持久,违规由新连接读不到判定 |
...
| **完整实验** | `run_full.sh` | 6 库 × 5 模型 × B0–B4 × 3 轮(63 任务) |
...
## 9. 局限与未来工作
- **completion-obligation 样本偏少**:已由 sqlitedict 覆盖(3 任务),但仅 1 库;可再引入连接/会话/锁类资源库扩样本。
- **规模**:63 任务、6 类全覆盖(completion-obligation 3,其余各 ≥11);可按真实约束扩到更大。
- **抽取评测**:抽取器 `pair_extractor.py` 已落地,RQ1 见 `docs/B5_成对约束抽取方法与RQ1.md`;gold 规则双人标注 + IAA 待补。
- **最终全量待跑**:`run_full.sh`(6 库 × 5 模型 × B0–B4 × 3 轮);sqlitedict 已锚定但尚未过模型生成,当前主结果为 5 库全量。
...
### 10.2 历史脉络
2026-06-12 执行 oracle 上线(P1)→ 06-13 taxonomy 扩到 6 类 → 06-14 通用 harness,先以流行真实库 msgspec/peewee/niquests 实测发现**强先验混淆**;改用自编合成库经复核发现**构造效度/循环性问题**;最终改用真实低流行度库(simplug/diot/simpleconf + pyparam),gold 规则从真实源码抽取 → **06-15**:① 发现并修复 simpleconf 裸调用 bug、simplug config-return B4 反降(分层 B4 + harness 按任务绑管理器)、测试隔离 bug,建立注入可执行性审计闸门;② **pyparam 因 B1 即满分(无组合缺口)退役**,**新增 glom(未见、提取域)与 bidict(高先验对照)**,主实验定型为 **5 库 60 任务**;③ B4 统一为**分层注入**(B1⊕该任务规则),核心指标改为 `B4−B1`。→ **06-16**:① simplug 迁入通用 benchlib(`lib_simplug.py`,复用 hidden_tests/anchor_solutions),退役其 pilot/exec_runner/run_exec_eval 老管线;② **新增 sqlitedict(持久化、写入须 commit 的 completion-obligation)补足第 6 类**,主实验定型为 **6 库 63 任务**、6 类全覆盖、锚定 153 例;③ 落地 B5 抽取器 `pair_extractor.py` + RQ1 + 失败反馈闭环(见 `docs/B5_成对约束抽取方法与RQ1.md`、`docs/paircoder数学原理.md`)。本文为权威说明。
