...
simplug keeps its own bespoke harness (fixture_simplug + hidden_tests +
anchor_solutions + exec_runner). The *new* cross-domain libraries (msgspec,
peewee, niquests) all share THIS generic harness, by each exposing a small
``lib_<name>`` module with a uniform interface:
...
def evaluate_source(lib, task_id: str, source: str) -> dict:
    ...
    Protocol (mirrors exec_runner.py for simplug):
...
def run_anchors(name: str, verbose: bool = True) -> bool:
    ...
    documented. This is the oracle's trustworthiness gate, same contract as
    simplug's run_exec_eval.run_anchors (canonical/variants PASS;
    every documented violation FAILS with an expected reason)."""
    lib = load_lib(name)
    rows = []
    all_ok = True
