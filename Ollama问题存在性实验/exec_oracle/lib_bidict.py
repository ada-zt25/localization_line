...
ORACLE_API_SEQUENCES = {
    "bidict-B001": "Oracle API sequence:\nbidict(pairs).inv[val]   # reverse lookup",
    "bidict-B002": "Oracle API sequence:\ndict(bidict(pairs).inv)   # value->key, as a plain dict",
    "bidict-B003": "Oracle API sequence:\nbidict(pairs).inv   # the inverse bidict",
    "bidict-B004": "Oracle API sequence:\nb = bidict(pairs); b.forceput(key, val); dict(b)",
    "bidict-B005": "Oracle API sequence:\nb = bidict(pairs); b[key] = val; b.inv[val]   # same b",
    "bidict-B006": "Oracle API sequence:\nb = bidict(pairs); b.inv[val] = key; b[key]   # same b",
    "bidict-B007": "Oracle API sequence:\nb = bidict(pairs); b.inv[b[key]]",
    "bidict-B008": "Oracle API sequence:\nb = bidict(pairs); try: b.putall(batch) except DuplicationError: pass; dict(b)",
    "bidict-B009": "Oracle API sequence:\nfrozenbidict(pairs)   # immutable",
    "bidict-B010": "Oracle API sequence:\nb = bidict(pairs); try: b[key] = val except ValueDuplicationError: pass; dict(b)",
}
