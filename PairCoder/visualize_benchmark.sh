...
# (works for all six benchlib libraries -- they all write the same CSV with
...
#   DIRS="result/glom_5model result/sqlitedict_5model" bash visualize_benchmark.sh
...
DIR_LIST=()
if [ -n "$DIRS" ]; then
  # user-provided, space-separated (pass paths WITHOUT spaces, e.g. result/glom_5model)
