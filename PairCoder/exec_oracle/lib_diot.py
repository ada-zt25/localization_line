...
# B3*: ABSTRACT call sequence -- API/order only; constraint-bearing options
# (diot_transform/diot_nest, same-object identity, thaw scope) left to the model.
ORACLE_API_SEQUENCES = {
    "diot-D001": "Abstract API sequence (call order only; you choose the constructor options):\nd = Diot(data, <options>); then access the requested key",
    "diot-D002": "Abstract API sequence (call order only; you choose the constructor options):\nd = Diot(data, <options>); then access the requested key",
    "diot-D003": "Abstract API sequence (call order only):\nd = Diot(data); then read the key with a fallback for the missing case",
    "diot-D004": "Abstract API sequence (call order only):\nd = Diot(data); then convert it to the required plain type",
    "diot-D005": "Abstract API sequence (call order only; you choose the constructor options):\nd = Diot(data, <options>); then index the nested value",
    "diot-D006": "Abstract API sequence (call order only):\nd = Diot(data); then access the nested value",
    "diot-D007": "Abstract API sequence (call order only):\nd = Diot(data); then navigate d -> nested -> field",
    "diot-D008": "Abstract API sequence (call order only):\nd = Diot(data); then convert, then index into the result",
    "diot-D009": "Abstract API sequence (call order only):\nd = Diot(...); write d[key]; then read d[key]",
    "diot-D010": "Abstract API sequence (call order only):\nd = Diot(data); mutate a nested value; then read it back",
    "diot-D011": "Abstract API sequence (call order only):\nd = FrozenDiot(data); modify it within the allowed scope; then read",
    "diot-D012": "Abstract API sequence (call order only):\nd = FrozenDiot(data); modify it within the allowed scope",
}
