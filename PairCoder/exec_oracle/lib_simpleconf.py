...
# (source merge order, same-conf identity, persistent-vs-temporary profile switch)
# left to the model.
ORACLE_API_SEQUENCES = {
    "simpleconf-S001": "Abstract API sequence (call order only; you choose the source order):\nConfig.load(<sources>); then read [key]",
    "simpleconf-S002": "Abstract API sequence (call order only; you choose the source order):\nConfig.load(<sources>); then read [key]",
    "simpleconf-S003": "Abstract API sequence (call order only):\nConfig.load(data); then use the returned config",
    "simpleconf-S004": "Abstract API sequence (call order only):\nProfileConfig.load(...); select a profile; then read conf.x",
    "simpleconf-S005": "Abstract API sequence (call order only):\nProfileConfig.load(...); select a profile; then read the active profile name",
    "simpleconf-S006": "Abstract API sequence (call order only):\nConfig.load(data); then read [key]",
    "simpleconf-S007": "Abstract API sequence (call order only):\nProfileConfig.load(...); then read conf.x",
    "simpleconf-S008": "Abstract API sequence (call order only):\nConfig.load(data); then read [k1][k2]",
    "simpleconf-S009": "Abstract API sequence (call order only):\nProfileConfig.load(...); switch the active profile; then read conf.x (same conf)",
    "simpleconf-S010": "Abstract API sequence (call order only):\nConfig.load(...); write conf[key]; then read conf[key]",
    "simpleconf-S011": "Abstract API sequence (call order only):\nload(...); switch profile; switch profile again; then read (same conf)",
    "simpleconf-S012": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); read conf.x inside a profile scope; then read conf.x after it",
    "simpleconf-S013": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); set a profile; read conf.x inside another profile scope; then read after",
    "simpleconf-S014": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); set a profile; inside a profile scope read the active profile name",
    "simpleconf-S015": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); nested profile scopes; read conf.x inside each and after",
    "simpleconf-S016": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); set a profile; enter a profile scope; then read conf.x after it",
    "simpleconf-S017": "Abstract API sequence (call order only; you choose persistent vs temporary switching):\nload(...); set a profile; set another profile; then read conf.x",
    "simpleconf-S018": "Abstract API sequence (call order only):\nload(...); set a profile; then read conf.x twice (same conf)",
}
