...
# B3*: ABSTRACT call sequence -- the APIs/order only, with the constraint-bearing
# spec/arguments left as placeholders (you choose them). Isolates "sequence" from
# "pair constraint" so B4-B3* cleanly measures the rule's marginal value.
ORACLE_API_SEQUENCES = {
    "glom-G001": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G002": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G003": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G004": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G005": "Abstract API sequence (call order only; you choose the spec/options):\nglom(data, <spec>, <options>)",
    "glom-G006": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G007": "Abstract API sequence (call order only):\nglom(data, <assign-spec>); then read the value back from the target",
    "glom-G008": "Abstract API sequence (call order only):\nglom(data, <assign-spec>); then return the target",
    "glom-G009": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
    "glom-G010": "Abstract API sequence (call order only; you choose the spec):\nglom(data, <spec>)",
}
